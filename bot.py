# bot.py — версия Yahoo DeMarker 28h (фикс FX/индексов/парсинга)
...
def norm_name(sym: str) -> str:
    ...
    # Коммодитис
    if sym in COMMO_MAP:
        return COMMO_MAP[sym]
    # Валюты: EURUSD=X -> EUR-USD
    if sym.endswith("=X") and len(sym) >= 7:
        pair = sym[:-2]  # 'EURUSD'
        base, quote = pair[:3], pair[3:]
        return f"{base}-{quote}".upper()
    s = sym.replace("^","")
    # Крипта: XXX-USD -> XXX-USDT
    if s.endswith("-USD"):
        return s[:-4] + "-USDT"
    # Акции: тикер -> тикер-USDT
